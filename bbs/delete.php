...
exit;
    ...
    else if ($member['mb_level'] < $mb['mb_level'])
    ...
    if ($write['mb_id']) {
        ...
        alert(
            '로그인 후 삭제하세요.',
            G5_BBS_URL.'/login.php?url='.urlencode(
...
sql_query("
    delete from {$g5['write_prefix']}messages
...
goto_url(
    short_url_clean(
        G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table
        .'&amp;page='.$page
        .$qstr
    )
);